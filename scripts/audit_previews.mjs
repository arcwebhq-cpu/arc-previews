...
assert.match(arc1, /checkoutPathMatchesMode/, "ARC1 does not bind the Payment Link URL to configured Stripe mode");
...
assert.match(arc1PaymentLink, /expand%5B%5D=line_items\.data\.price\.product/, "ARC1 Payment Link preflight does not expand the exact line item Price and Product");
assert.match(arc1PaymentLink, /https:\/\/api\.stripe\.com\/v1\/account/, "ARC1 Payment Link preflight does not verify the authenticated ARC Stripe account");
assert.match(arc1PaymentLink, /expectedStripeAccountIdSha256/, "ARC1 Payment Link preflight does not bind the private ARC Stripe account hash");
assert.match(arc1PaymentLink, /paymentLink\.automatic_tax\?\.enabled !== true/, "ARC1 Payment Link preflight does not require Stripe automatic tax");
...
assert.match(arc2, /expected Stripe Tax registration is not active/, "ARC2 does not re-read the configured Stripe Tax registrations");
...
assert.match(arc2EmailGate, /arc2-claim-state-evidence-signature-v2/, "ARC2 email lacks signed deploy-and-claim state evidence");
...
assert.match(arc2EmailGate, /exact Payment Link, Price, Product tax code, and static terms version/, "ARC2 email does not independently bind exact Stripe configuration");
