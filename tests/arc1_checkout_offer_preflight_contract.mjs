...
const source = await readFile(new URL("../zapier/arc1_verify_checkout_offer.js", import.meta.url), "utf8");
...
const priceId = "price_1ArcV11Test5000";
...
const input = {
  ...
  checkout_offer_evidence_secret: "arc-test-offer-evidence-secret-32-bytes-minimum"
};
const urls = {
...
assert.equal(verified.status, "PRIVATE_CHECKOUT_SESSION_OFFER_VERIFIED");
assert.equal(verified.external_write_allowed_by_this_step, false);
assert.equal(Object.hasOwn(verified, "checkout_url"), false);
assert.equal(Object.hasOwn(verified, "checkout_session_id"), false);
const evidence = JSON.parse(verified.checkout_offer_evidence_private);
assert.equal(evidence.version, "arc1-checkout-offer-evidence-v2");
assert.equal(evidence.scope, "authoritative-private-checkout-session-offer-preflight");
...
assert.match(verified.checkout_offer_evidence_sha256, /^[a-f0-9]{64}$/);
assert.match(verified.checkout_offer_evidence_hmac_sha256, /^[a-f0-9]{64}$/);
...
console.log("ARC1 signed Checkout Session offer preflight contract passed (zero checkout capability).");
