...
const stableConfiguration={stripe_account_id_sha256:accountSha,livemode:false,price_id:"price_ArcLifecycle5000",product_id:"prod_ArcLifecycleWebsite",
  amount_subtotal_minor_units:500000,currency:"usd",quantity:1,terms_version:"2026-08-12",terms_document_sha256:"c".repeat(64),automatic_tax_enabled:true,
...
const offer=canonical({version:"arc-checkout-offer-snapshot-v1",scope:"immutable-approved-preview-private-checkout-offer",environment:"arc-production",checkout_binding_key_id:kid,
  preview_folder:"summit-roofing-a1b2c3d4",preview_path:"summit-roofing-a1b2c3d4/index.html",preview_source_repository:"arcwebhq-cpu/arc-previews",public_folder_prefix:"a1b2c3d4",
...
const policy=canonical({version:"arc-private-checkout-policy-v1",scope:"one-approved-preview-one-private-payment-link",stripe_mode:mode,
  stripe_account_id_sha256:accountSha,price_id:offerObject.price_id,product_id:offerObject.product_id,product_tax_code:offerObject.product_tax_code,
  automatic_tax_enabled:true,payment_method_selection:"dynamic",completed_sessions_limit:1,readiness_core_sha256:readinessCoreSha,offer_snapshot_sha256:offerSha});
...
const readinessAt=issuedMs=>{
  const raw=canonical({version:"arc1-preview-readiness-observation-v1",scope:"renewable-private-checkout-readiness-observation",readiness_core_sha256:readinessCoreSha,
    current_main_sha:"4".repeat(40),current_main_html_sha256:"5".repeat(64),pages_content_sha256:"5".repeat(64),issued_at:iso(issuedMs),expires_at:iso(issuedMs+10*60*1000)});
  return{raw,hmac:mac(bindingSecret,`arc1-preview-readiness-observation-signature-v1\n${mode}\n${raw}`)};
};
...
const finalOffer=offerEvidenceAt(now-5000),finalReadiness=readinessAt(now-4000);
