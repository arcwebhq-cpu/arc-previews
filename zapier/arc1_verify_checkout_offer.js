// ARC1 Checkout Session offer preflight — authenticates the ARC account, Price,
// Product, tax registrations, and retained terms. It performs read-only GETs
// against those exact Stripe resources and never reads or creates checkout capability.
...
const evidenceSecret = clean(inputData.checkout_offer_evidence_secret);
...
if (!/^price_[A-Za-z0-9]+$/.test(expectedPriceId)) {
  throw new Error("ARC_CHECKOUT_OFFER_PREFLIGHT_INVALID: exact Price id is required");
}
...
if (expectedTermsVersion !== requiredTermsVersion || !/^[a-f0-9]{64}$/.test(expectedTermsDocumentSha256) ||
  ...
  throw new Error("ARC_CHECKOUT_OFFER_PREFLIGHT_INVALID: retained immutable terms document registry mismatch");
}
if (expectedRedirectUrl !== requiredCheckoutRedirectUrl) {
  throw new Error("ARC_CHECKOUT_OFFER_PREFLIGHT_INVALID: checkout redirect must match the static ARC payment-success URL");
}
let expectedRedirect;
try {
  expectedRedirect = new URL(expectedRedirectUrl);
} catch (error) {
  throw new Error("ARC_CHECKOUT_OFFER_PREFLIGHT_INVALID: configured URL");
}
if (expectedRedirect.protocol !== "https:" || expectedRedirect.username || expectedRedirect.password || expectedRedirect.hash ||
  ...
  throw new Error("ARC_CHECKOUT_OFFER_PREFLIGHT_INVALID: exact HTTPS redirect URL is required");
}
if (evidenceSecret.length < 32 || evidenceSecret.length > 256) {
  throw new Error("ARC_CHECKOUT_OFFER_PREFLIGHT_INVALID: evidence secret must be 32–256 characters");
}
...
if(!Number.isSafeInteger(operationTimeoutMs)||operationTimeoutMs<25||operationTimeoutMs>25_000)throw new Error("ARC_CHECKOUT_OFFER_PREFLIGHT_INVALID: provider deadline");
...
const stripeGet = async (resourceUrl,maximumBytes) => {
  const remaining=Math.floor(operationDeadline-Date.now());if(remaining<=0)throw new Error("ARC_CHECKOUT_OFFER_PREFLIGHT_INVALID: provider operation deadline exceeded");
  ...
  try{
    ...
    if(Date.now()>=operationDeadline)throw new Error("ARC_CHECKOUT_OFFER_PREFLIGHT_INVALID: provider operation deadline exceeded");
    if(response.url&&response.url!==resourceUrl)throw new Error("ARC_CHECKOUT_OFFER_PREFLIGHT_INVALID: Stripe API redirect rejected");
    if(!response.ok)throw new Error(`ARC_CHECKOUT_OFFER_PREFLIGHT_INVALID: Stripe read failed (${response.status})`);
    const declared=response.headers?.get?.("content-length");if(declared&&(!/^\d{1,9}$/.test(declared)||Number(declared)>maximumBytes))throw new Error("ARC_CHECKOUT_OFFER_PREFLIGHT_INVALID: Stripe response too large");
    reader=response.body?.getReader?.();if(!reader)throw new Error("ARC_CHECKOUT_OFFER_PREFLIGHT_INVALID: streaming Stripe response required");
    let total=0;const chunks=[];while(true){const {done,value}=await reader.read();if(done)break;if(!(value instanceof Uint8Array))throw new Error("ARC_CHECKOUT_OFFER_PREFLIGHT_INVALID: invalid Stripe response chunk");total+=value.byteLength;if(total>maximumBytes){try{await reader.cancel();}catch{}throw new Error("ARC_CHECKOUT_OFFER_PREFLIGHT_INVALID: Stripe response too large");}chunks.push(Buffer.from(value));}
    try{return JSON.parse(Buffer.concat(chunks,total).toString("utf8"));}catch{throw new Error("ARC_CHECKOUT_OFFER_PREFLIGHT_INVALID: Stripe response JSON");}
  }catch(error){if(error?.name==="AbortError")throw new Error("ARC_CHECKOUT_OFFER_PREFLIGHT_INVALID: Stripe bounded timeout");throw error;}finally{clearTimeout(timer);try{reader?.releaseLock?.();}catch{}}
};
...
if (!price || typeof price !== "object" || Array.isArray(price) ||
  ...
  throw new Error("ARC_CHECKOUT_OFFER_PREFLIGHT_INVALID: Price and Product do not match the exact private Checkout Session offer");
}
...
try {
  ...
  throw new Error("ARC_CHECKOUT_OFFER_PREFLIGHT_INVALID: checkout redirect URL");
}
if (redirectUrl !== expectedRedirectUrl || redirect.username || redirect.password || redirect.hash ||
  ...
  throw new Error("ARC_CHECKOUT_OFFER_PREFLIGHT_INVALID: redirect must exactly match the configured HTTPS URL and Checkout Session placeholder");
}
...
const signature = await globalThis.crypto.subtle.sign(
  "HMAC", evidenceKey, encoder.encode(`arc1-checkout-offer-evidence-signature-v2\n${evidencePrivate}`)
);
return {
  status: "PRIVATE_CHECKOUT_SESSION_OFFER_VERIFIED",
